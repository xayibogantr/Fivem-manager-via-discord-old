    ...
    if #result > 0 then
        for i = 1, #result do
            ...
            if #embed.fields >= 24 then
                TriggerEvent('user0919-db:SendEmbed', embed)
                embed.fields = {}
            end
        end
        TriggerEvent('user0919-db:SendEmbed', embed)
        ...
        local embed = {
            description = "Yasaklanmış oyuncu bulunamadı.",
            color = "#ff0000",
            author = 'HATA'
        }
        TriggerEvent('user0919-db:SendEmbed', embed)
...
exports('GetBannedPlayer', function(id)
    if GetPlayerName(id) then
        ...
        if result and result[1] then
            ...
            local embed = {
                description = "Kullanıcı yasaklı değil.",
                color = "#33ff00",
                author = 'BILGI'
            }
            TriggerEvent('user0919-db:SendEmbed', embed)
    ...
    if Config.useQB then
        if GetPlayerName(id) then
            ...
            if result and result[1] then
                ...
                TriggerEvent('user0919-db:SendEmbed', embed)
                DropPlayer(id, 'Karakteriniz silindi.') 
            else
                local embed = {
                    description = "Kullanıcı bulunamadı.",
                    color = "#ff0000",
    ...
    if Config.useQB then
        ...
        if GetPlayerName(id) and QBCore.Functions.GetPlayer(tonumber(id)) then
            ...
            if inventory and next(inventory) then
                ...
                embed.description = 'Herhangi bir eşyası bulunamadı.'
            end
...
exports('SetCoords', function (id, x, y, z)
    if GetPlayerName(id) then
        ...
        local embed = {
            description = "Oyuncu aktif değil.",
            color = "#ff0000",
            author = 'HATA'
        }
        TriggerEvent('user0919-db:SendEmbed', embed)
        ...
        if GetPlayerName(id) then
            ...
            if result and result[1] then
                ...
                if result[1].money and json.decode(result[1].money) then
                    ...
                    local embed = {
                        description = "[DATABASE] Kullanıcı bulunamadı.",
                        color = "#ff0000",
                        author = 'HATA'
                    }
                    TriggerEvent('user0919-db:SendEmbed', embed)
    ...
    if Config.useQB then
        ...
        if GetPlayerName(id) then
            ...
            if xPlayer then
                ...
                local embed = {
                    color = "#0094ff", -- blue
                    title = '`'..GetPlayerName(id)..'` verildi `'..amount..'` \nEski Miktar: `'..xPlayer.PlayerData.money[type]..'` \nSTIL: `'..type..'`',
                }
                TriggerEvent('user0919-db:SendEmbed', embed)
            else   
                local embed = {
                    description = "[ESX] Kullanıcı bulunamadı.",
                    color = "#ff0000",
                    author = 'HATA'
                }
                TriggerEvent('user0919-db:SendEmbed', embed)
    ...
    if Config.useQB then
        if GetPlayerName(id) then
            ...
            if xPlayer then
                ...
                local beforeJobGrade = xPlayer.PlayerData.job.grade
                xPlayer.Functions.SetJob(job, grade)
                local embed = {
                    color = "#0094ff", -- blue
                    title = GetPlayerName(id)..' mesleği değiştirildi. \nEski Meslek: `'..beforeJob..'`\nYeni Meslek: `'..job..'` \nEski Rütbe: `'..beforeJobGrade..'` \nYeni Rütbe: `'..grade..'`',
                }
                TriggerEvent('user0919-db:SendEmbed', embed)
            else   
                local embed = {
                    description = "QBCORE Kullanıcı bulunamadı.",
                    color = "#ff0000",
                    author = 'HATA'
                }
                TriggerEvent('user0919-db:SendEmbed', embed)
        ...
        TriggerEvent('esx:getSharedObject', function(ESX)
            if ESX.DoesJobExist(job, grade) then
                ...
                local embed = {
                    description = "[ESX] İş, seviye veya her ikiside geçersiz.",
                    color = "#ff0000",
                    author = 'HATA'
                }
                TriggerEvent('user0919-db:SendEmbed', embed)
    ...
    if Config.useQB then
        ...
        if GetPlayerName(id) then
            ...
            if xPlayer then
                ...
                local embed = {
                    color = "#0094ff", -- blue
                    title = '`'..GetPlayerName(id)..'` alındı `'..amount..'` \nEski Miktar: `'..xPlayer.PlayerData.money[type]..'` \nSTIL: `'..type..'`',
                }
                TriggerEvent('user0919-db:SendEmbed', embed)
            else   
                local embed = {
            ...
            if result and result[1] then
                if result[1].money and json.decode(result[1].money) then
                    ...
                    local embed = {
                        color = "#0094ff", -- blue
                        title = '`'..id..'` alındı `'..tonumber(amount)..'` \nEski Miktar: `'..xPlayer.PlayerData.money[type]..'` \nSTIL: `'..type..'`',
                    }
                    TriggerEvent('user0919-db:SendEmbed', embed)
                else    
                    local embed = {
                    ...
                    TriggerEvent('user0919-dbt:SendEmbed', embed)
                end
            else
                local embed = {
                    description = "Kullanıcı bulunamadı.",
...
exports('GiveItem', function (id, name, count)
    ...
    if Config.useQB then
        if GetPlayerName(id) then
            ...
            local embed = {
                color = "#0094ff", -- blue
                title = '`'..GetPlayerName(id)..'` eşya verildi. \nEşya Ismi:`'..name..'` \nMiktar: `'..count..'`',
            }
            TriggerEvent('user0919-db:SendEmbed', embed)
        else
            local embed = {
...
exports('GetPlayers', function ()
    ...
    if Config.useQB then
        ...
        embed.title = '`'..#players..'` Aktif Oyuncular'
        if next(players) then
            embed.fields = {
                {name = 'ISIM [ID]', inline = true, value = ''},
...
exports('Revive', function(id)    
    if Config.useQB then 
        if GetPlayerName(id) then
            id = tonumber(id)
            local QBCore = exports['qb-core']:GetCoreObject()
            local xPlayer = QBCore.Functions.GetPlayer
            TriggerClientEvent('esx_ambulancejob:revive', id, true)
        else
        ...
        local embed = {
            description = "Oyuncu oyunda değil.",
            color = "#ff0000",
            author = 'HATA'
        }
        TriggerEvent('user0919-db:SendEmbed', embed)
...
exports('ReviveAll', function ()
    if Config.useQB then
        ...
        if next(players) then
            local embed = {
                color = "#0094ff", -- blue
                title = 'Tüm kullanıcılar tekrardan iyileştirildi. TOPLAM:`'..#players..'`',
            }
            TriggerEvent('user0919-db:SendEmbed', embed)
        else    
            local embed = {
...
exports('spawnCar', function (id, name)
    if Config.useQB then 
        ...
        local embed = {
            color = "#0094ff", -- blue
            title = ' `'..name..'` verildi `'..GetPlayerName(id)..'`',
        }
        TriggerEvent('user0919-db:SendEmbed', embed)
    else    
        local embed = {
...
exports('spawnAllPlayersCar', function (name)
    if Config.useQB then
        ...
        if #players > 0 then
            ...
            local embed = {
                color = "#0094ff", -- blue
                title = ' Toplam Verilen Araç: `'..#players..'`',
            }
            TriggerEvent('user0919-db:SendEmbed', embed)
        else    
            local embed = {

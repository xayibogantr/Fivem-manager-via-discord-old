...
client.on('ready', () => {
     ...
     exports['user0919-db'].GetConfig(config);
     if (config.discord_whitelist.active) {
          on('playerConnecting', (name, setCallback, deferrals) => {
               ...
               deferrals.defer();
               const player = exports['user0919-db'].GetDiscordFromId(src)
...
client.on('message', msg => {
     ...
     const embed = new Discord.MessageEmbed().setFooter('user0919 ❤️ ')
          ...
          if (msg.member.roles.find((search) => search.id == config.admin_roleid)) {
               if (args[0] == config.prefix + 'ban') {
                    if (args[1] && args[2] && args[3]) {
                         emit('user0919-db:Ban', msg.member.user, args[1], args, (Number(args[2]) && args[2] || 0))
                         return false;
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    if (args[1] && args[2]) {
                         emit('user0919-db:Kick', msg.member.user, args[1], args)
                         return false;
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    if (args[1]) {
                         return exports['user0919-db'].GetInventory(args[1]);
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    if (args[1]) {
                         return exports['user0919-db'].start(args[1]);
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    if (args[1]) {
                         return exports['user0919-db'].stop(args[1]);
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    if (args[1]) {
                         return exports['user0919-db'].GetMoney(args[1]);
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    if (args[1] && args[2] && args[3]) {
                         if (args[2] == 'money' || args[2] == 'bank' || (args[2] == 'black_money' || args[2] == 'crypto')) {
                              return exports['user0919-db'].AddMoney(args[1], args[2], (Number(args[3]) && args[3] || 0));
                         } else {
                              embed
                                   .setColor('#ff0000')
                    ...
                    if (args[1] && args[2] && args[3]) {
                         if (Number(args[3]) != null) {
                              return exports['user0919-db'].GiveItem(args[1], args[2], Number(args[3]));
                         } else {
                              embed
                                   .setColor('#ff0000')
                    ...
                    if (args[1] && args[2] && args[3]) {
                         if (Number(args[3]) != null) {
                              return exports['user0919-db'].SetJob(args[1], args[2], Number(args[3])); 
                         } else {
                              embed
                                   .setColor('#ff0000')
                    ...
                    if (args[1] && args[2] && args[3]) {
                         if (args[2] == 'money' || args[2] == 'bank' || args[2] == 'black_money') {
                              return exports['user0919-db'].RemoveMoney(args[1], args[2], (Number(args[3]) && args[3] || 0));
                         } else {
                              embed
                                   .setColor('#ff0000')
                                   .addField('ID', 'Lütfen kullanıcı ID`si belirle.', true)
                    ...
                    if (args[1]) {
                         return exports['user0919-db'].GetBannedPlayer(args[1])
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    return exports['user0919-db'].ReviveAll(); 
                                   } else if (args[0] == config.prefix + 'wlver') { //
                    const member = msg.mentions.members.first();
                    if (!member) {
                         embed
                         .setColor('#ff0000')
                         .setTitle('Kullanım: !wlver `@user0919`')
                         .setAuthor('Yanlış Komut Kullanımı')
                         msg.channel.send(embed)
                         return;
                    }
                    ...
                    embed
                         ...
                         if (!member) {
                              embed
                              .setColor('#ff0000')
                              .setTitle('Kullanım: !wlsil `@user0919`')
                              .setAuthor('Yanlış Komut Kullanımı')
                              msg.channel.send(embed)
                              return;
                         }
                    ...
                    if (args[1]) {
                         return exports['user0919-db'].Revive(args[1]);
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    if (!member) {
                         embed
                         .setColor('#ff0000')
                         .setTitle('Kullanım: !removewl `@user0919`')
                         .setAuthor('Yanlış Komut Kullanımı')
                         msg.channel.send(embed)
                         return;
                    }
                    ...
                    return exports['user0919-db'].GetBannedPlayers();
               } else if (args[0] == config.prefix + 'oyuncular') {
                    return exports['user0919-db'].GetPlayers();
               } else if (args[0] == config.prefix + 'arabaver') {
                    if (args[1] && args[2]) {
                         return exports['user0919-db'].spawnCar(args[1], args[2]);
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    if (args[1]) {
                         return exports['user0919-db'].spawnAllPlayersCar(args[1]);
                    } else {
                         embed
                              .setColor('#ff0000')
                    ...
                    if (args[1] && args[2] && args[3]) {
                         return exports['user0919-db'].giveWeapon(args[1], args[2], args[3]);
                    } else {
                         embed
                              .setColor('#ff0000')
               ...
               embed
                    .setColor('#ff0000')
                    .setDescription('Herhangi bir yetkin yok. user0919`e ulaş.')
                    .setAuthor('HATA')
               msg.channel.send(embed)
               return false;
          }
...
RegisterNetEvent('user0919-db:SendEmbed')
on('user0919-db:SendEmbed', embed => CurrentChannel?.send(SendEmbed(embed)));

SendEmbed = (embed) => {
     const CreateEmbed = new Discord.MessageEmbed().setFooter('user0919 ❤️ ')
